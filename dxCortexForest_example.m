% This script gives an example of training a dxCortex classifier augmented with in-bag decision trees like those used in the 
% top classifiers of the hierarchical COVID-19 prognostic tests.  
...
% these are the additional attributes that will be used in the in-bag decision trees
tree_ftrs = {
    'Race',
    'age',
    'sex_or_gender',
...
% marks which features for the tree are categorical
tree_ftrs_categorical = {true, false, true, false};

% hyper parameters for the bagging, while nmimibags=10 was used in the COVID-19 prognostic tests study, nbags is set to 50
% for this example so this script will run in a sensible amount of time on a single core.  For the COVID-19 prognostic tests study,
% these methods implemented in an HPC environment were used with nbags=625 and nminibags=30
nbags = 50;
nminibags = 10;
...
dcbs = cell(nbags,1); % cell array of dxCortexBags to save for predicting on validation data
trees = cell(nbags,1); % cell array of cell arrays of trees, one cell array per dxCortexBag containing the trees for that bag

for ibag = 1:nbags
    fprintf('on bag: %d out of %d\n', ibag, nbags);

    % generate this bag's train/test split
    se.generate_bag_sampling(0.333);
    ...
    % predict on this bags test set with the dxCortex MC
    [~, oob_labels] = dcb.predict_master_classifier(se.get_test_data(dxCortex_ftrs).data, 0.5);

    % set up sampling_engine for trees and loop over minibags
    se.prep_data_for_trees(oob_labels, tree_ftrs, tree_ftrs_categorical);
    these_trees = cell(nminibags,1);
    for imb = 1:nminibags
        se.generate_minibag_sampling(0.5);

        this_crast = crast(se.get_train_data_tree(), 'use_weighted_ave_info_gain', true, 'min_leaf_size', 1, 'is_feature_categorical', se.tree_ftrs_categorical);
        this_crast.fit_tree();

        % save this tree
        these_trees{imb} = this_crast;

        % oob predictions to get reliable classifications on the development set
        oob_labels = this_crast.predict_tree(se.get_test_data_tree());
        se.set_bag_labels(oob_labels);
    end
    trees{ibag} = these_trees;
end
...
% need both sets of features, can be named differently in validation data spreadsheet, but...
...
% marks which features for the tree are categorical
% redeclaring here for clarity, but this should be identical to its previous declaration
tree_ftrs_categorical = {true, false, true, false};
...
for ibag = 1:length(dcbs)
    ...
    % prep the validation data for the trees
    se.prep_val_data_for_trees(val_labels, tree_ftrs, tree_ftrs_categorical);

    % get the trees in this bag
    these_trees = trees{ibag};
    for imb = 1:length(these_trees)
        this_crast = these_trees{imb};
        val_labels = this_crast.predict_tree(se.get_val_data_tree());
        results = [results, val_labels];
    end
end

% results is now a [nsamples, nbags*nminibags] matrix of labels, need to vote to get final labels
[nsamples, ntrees] = size(results);
val_labels = -9999*ones(nsamples,1);
for isample = 1:nsamples
    n0s = 0;
    n1s = 0;
    for itree = 1:ntrees
        if results(isample, itree) > 0.5
            n1s = n1s + 1;
        else
            n0s = n0s + 1;
        end
